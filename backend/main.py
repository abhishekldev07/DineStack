...
def ensure_order_location_columns():
    with engine.begin() as connection:
        connection.execute(
            text(
                "ALTER TABLE orders ADD COLUMN IF NOT EXISTS phone_number VARCHAR"
            )
        )
        connection.execute(
            text(
...
def ensure_user_verification_columns():
    with engine.begin() as connection:
        ...
        connection.execute(
            text(
                "UPDATE users SET is_verified = TRUE WHERE verification_token IS NULL AND verification_token_expires IS NULL"
            )
        )
        connection.execute(
            text(
...
def ensure_reservation_columns():
    with engine.begin() as connection:
        connection.execute(
            text(
                "ALTER TABLE reservations ADD COLUMN IF NOT EXISTS payment_status VARCHAR DEFAULT 'pending_payment'"
            )
        )
        connection.execute(
            text(
                "ALTER TABLE reservations ADD COLUMN IF NOT EXISTS payment_method VARCHAR"
            )
        )
        connection.execute(
            text(
                "ALTER TABLE reservations ADD COLUMN IF NOT EXISTS transaction_id VARCHAR"
            )
        )
        connection.execute(
            text(
                "ALTER TABLE reservations ADD COLUMN IF NOT EXISTS paid_amount FLOAT DEFAULT 0"
            )
        )
        connection.execute(
            text(
                "ALTER TABLE reservations ADD COLUMN IF NOT EXISTS paid_at TIMESTAMP WITH TIME ZONE"
            )
        )
